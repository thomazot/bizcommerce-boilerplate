...
$j(document)
    .ready(function ($) {
        ...
        $('.filters__name').click(function () {
            $(this).parent('.filters__filter').toggleClass('on')
        })

        const categoryTitle = $('.catalog-category-view .page-title')

        if (categoryTitle.length) {
            if (categoryTitle.next().next('.toolbar').length) {
                categoryTitle
                    .next()
                    .next('.toolbar')
                    .attr('data-title', $.trim(categoryTitle.text()))
            }
        }
